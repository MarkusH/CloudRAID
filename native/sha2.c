...
typedef unsigned long sha2_word64; /* Exactly 8 bytes */
 ...
 * by the CloudRAID team and are NOT part of the original APR
...
{
    size_t l;
    unsigned char buf[SHA256_BLOCK_LENGTH];
    sha256_ctx context;
    sha256_init(&context);
    ...
    while(!feof(fp)) {
        l = fread(buf, sizeof(unsigned char), SHA256_BLOCK_LENGTH, fp);
        sha256_update(buf, l, &context);
    }

    sha256_end(hash, &context);
...
LIBEXPORT int build_sha256_sum(char *filename, unsigned char *hash)
{
    ...
    return build_sha256_sum_file(fp, hash);
}
